// 1. read passport batch.  store as an array of maps.
// 2. check each passport against rule.
...
let realInput = ["eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241",
    "",
...
// returns an array of maps.  Each map represents one passport.
// iterate through input.
function genPassArray(input) {
    let passArray = [];
    passArray.push(new Map());
    for (const passItem of input) {
        // if current pass-item is not empty, then add key-value pairs to current map.
        if (passItem) {
            // separate passport fields into array.
            let a = passItem.split(' ');
            // add key-value pairs to current map (i.e. last item in passArray).
            while (a.length > 0) {
                let passField = a.pop();
                let key = passField.slice(0, passField.indexOf(":"));
                let val = passField.slice(passField.indexOf(":") + 1);
                passArray[passArray.length - 1].set(key, val);
            }
            // else if current pass-item is empty, then push new map to pass-array.
        } else if (!passItem) {
            // add new map to passArray
            passArray.push(new Map());
        } else {
...
// checks for presence of keys: byr, iyr, eyr, hgt, hcl, ecl, pid.
// presence of key cid is optional.
function hasRequiredFields(passport) {
    const requiredKeys = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"];
    for (let requiredKey of requiredKeys) {
        if (passport.has(requiredKey)) {
            continue;
        } else {
            return false;
        }
...
function hasValidFields(passport) {
    ...
    let ecl = p.get('ecl');
    let pid = p.get('pid');
    return byrValid(byr) && iyrValid(iyr) && eyrValid(eyr) && hgtValid(hgt) && hclValid(hcl) && eclValid(ecl) && pidValid(pid);
}
...
function byrValid(byr) {
    ...
    if (byr.length != 4) {
        return false;
    } else if (Number(byr) >= 1920 && Number(byr) <= 2002) {
        return true;
    } else {
        return false;
    }
...
function iyrValid(iyr) {
    ...
    if (iyr.length != 4) {
        return false;
    } else if (Number(iyr) >= 2010 && Number(iyr) <= 2020) {
        return true;
    } else {
        return false;
    }
...
function eyrValid(eyr) {
    ...
    if (eyr.length != 4) {
        return false;
    } else if (Number(eyr) >= 2020 && Number(eyr) <= 2030) {
        return true;
    } else {
        return false;
    }
...
function hgtValid(hgt) {
    hgt = hgt.toString();
    // does the string end with cm or in?
    if (hgt.endsWith('cm')) {
        let h = Number(hgt.slice(0, hgt.length - 2));
        if (h >= 150 && h <= 193) {
            return true;
        } else {
            return false;
...
let passports = genPassArray(realInput);
// // console.log(passports);

let numValid = 0;
for (let pass of passports) {
    numValid += hasRequiredFields(pass) && hasValidFields(pass) ? 1 : 0;
}
