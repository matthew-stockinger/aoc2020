let pass1 = "1-3 a: abcde";
let testInput = ["1-3 a: abcde",
    "1-3 b: cdefg",
    "2-9 c: ccccccccc"];

let c2Input = ["9-10 b: bbktbbbxhfbpb",
"2-10 x: xxnxxxwxxsx",
"5-7 w: ghwwdrr",
"4-6 z: nzzjzk",
"7-8 s: szsssswfs",
...
function checkPassOld(s) {
    // first, read min value and max value, store in variables.
    let min = s.slice(0, s.indexOf('-'));
    let max = s.slice(s.indexOf('-') + 1, s.indexOf(' '));

    // read and store rule character
    let testChar = s.charAt(s.indexOf(" ") + 1);

    // prep regex and password string
    let testString = s.slice(s.indexOf(':') + 2);
    let re = new RegExp(testChar, 'g');

    // test password string against rule.  add to valid password count if passes.
    // return count.
    let count = testString.match(re);
    count = count ? count.length : 0;
    return count >= min && count <= max;
}

function checkPassNew(s) {
    // first, read min value and max value, store in variables.
    let pos1 = s.slice(0, s.indexOf('-'));
    let pos2 = s.slice(s.indexOf('-') + 1, s.indexOf(' '));
    let index1 = pos1 - 1;
    let index2 = pos2 - 1;
    ...
    // prep regex and password string
    let testString = s.slice(s.indexOf(':') + 2);
    let testSubString = testString.charAt(index1) + testString.charAt(index2);

    // is there exactly 1 occurrence?
    let re = new RegExp(`${testChar}[^${testChar}]|[^${testChar}]${testChar}`, 'g');

    // test password string against rule.  add to valid password count if passes.
    // return count.
    return re.test(testSubString);
}

let validPasswordCount = 0;
c2Input.forEach((entry) => {
    validPasswordCount += checkPassNew(entry) ? 1 : 0;
});
