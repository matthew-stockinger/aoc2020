// 26 questions.  which questions get a yes from anyone in my group?
// double newline indicates new group.

// 1. separate input into groups.
// 2. for each group, make set of 'yes' questions.  count them.
// 3. sum up counts.
...
class planeDeclarations {
    constructor(input) {
        this.decsByGroup = input
            .split(/\r?\n\r?\n/g)
            .map(group => group.split(/\r?\n/g)); // array of group arrays.  each group array is strings.
        this.uniqueDecsByGroup = input
            .split(/\r?\n\r?\n/g)  // array of group strings
            .map(groupString => groupString.replace(/\r?\n/g, '')) // strip newlines
            .map(groupString => new Set(groupString)); // array of sets.  removes duplicate characters in each group.
    }

    get totalYesCount() {
        return this.uniqueDecsByGroup
            .reduce((accumulator, nextSet) => accumulator + nextSet.size, 0);
    }

    get everyoneYesCount() {
        let res = 0;
        // in each group
        for (let i = 0; i < this.decsByGroup.length; i++) {
            // for each unique character
            for (let char of this.uniqueDecsByGroup[i]) {
                // add to count if character is present in every dec of the group.
                res += this.decsByGroup[i].every(dec => dec.includes(char)) ? 1 : 0;
            }
        }
        return res;
    }
...
let p = new planeDeclarations(realInput);
console.log(p.everyoneYesCount);
console.log();