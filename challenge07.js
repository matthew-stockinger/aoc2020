...
2.) generate a list of notes.  Set or array.
3.) connect up the children.  store them in a node.children array.
4.) make methods for moving up or down the node tree, and counting.
*/
...
class Bag {
    constructor(ruleString) {
        this.color = ruleString.slice(0, ruleString.search('contain') - 6);
        this.contains = this.parseChildren(ruleString);
    }

    parseChildren(ruleString) {
        return ruleString
            .slice(ruleString.indexOf('contain') + 8)
            .split(', ')
            .map(childString => {
                if (childString.endsWith('no other bags')) {
                    return {};
                } else {
                    return [
                        childString.slice(0, childString.indexOf(' ')),
                        childString.slice(childString.indexOf(' ') + 1, childString.indexOf('bag') - 1)
                    ];
                }
            });
    }
...
// change the contains property to contain the actual bag objects.
function makeChildConnections() {
    for (let bag of bags) {
        let res = [];
        // for each child
        for (let child of bag.contains) {
            // add appropriate number of references to child objects.
            for (let i = 0; i < child[0]; i++) {
                res.push(getBag(child[1], bags));
            }
        }
        bag.children = res;
    }
...
function getBag(bagColor, bagsArray) {
    return bags.find(bag => bag.color === bagColor);
}

function findParents(childBag) {
    let res = [];
    for (let bag of bags) {
        if (bag.children.includes(childBag)) {
            res.push(bag);
        }
    }
    return res;
...
// returns *set* of ancestor bags.
function findAllAncestors(childBag) {
    let res = new Set();
    for (let bag of bags) {
        if (bag.children.includes(childBag)) {
            res.add(bag);
            let grandparents = findAllAncestors(bag);
...
// returns array of descendant Bags.
function findAllDescendants(parentBag) {
    let res = [];
    for (let child of parentBag.children) {
        res.push(child);
        if (child.children.length > 0) {
            res.push(...findAllDescendants(child));
        }
    }
    return res;
}
...
// make array of rule strings.
let ruleList = makeRuleList(realInput);

// instantiate all bag objects into an array.
const bags = ruleList.map(ruleString => new Bag(ruleString));
makeChildConnections();

// need to traverse up.  how many nodes are above shiny gold?
// First, which nodes contain shiny gold?
// check all bags' children.
let sAncestors = findAllAncestors(getBag('shiny gold', bags));

// Part 2: find how many child bags there are, all the way down.
let sDescendants = findAllDescendants(getBag('shiny gold', bags));
console.log(sDescendants.length);
console.log();
